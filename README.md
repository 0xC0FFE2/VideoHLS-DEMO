...
- 강의 목차 기능 (강의를 여러 비디오가 포함된 목차바 형태로 구성)
- 시청 진행 상태 저장
...
### 강의 목차(Video Chapter) API
- `GET /api/videoChapters`: 모든 강의 목차 조회
  - Response: `[{ id, title, description, sortOrder, courseId, videos, ... }]`

- `GET /api/videoChapters/:id`: 목차 상세 조회
  - Response: `{ id, title, description, sortOrder, courseId, videos, ... }`

- `GET /api/videoChapters/course/:courseId`: 특정 강의의 모든 목차 조회
  - Response: `[{ id, title, description, sortOrder, courseId, videos, ... }]`

- `POST /api/videoChapters`: 목차 생성 (인증 필요, 관리자/강사 권한)
  - Headers: `Authorization: Bearer {token}`
  - Request Body: `{ title, description?, sortOrder?, courseId }`
  - Response: 생성된 목차 객체

- `PUT /api/videoChapters/:id`: 목차 업데이트 (인증 필요, 관리자/강사 권한)
  - Headers: `Authorization: Bearer {token}`
  - Request Body: `{ title?, description?, sortOrder?, isActive? }`
